...
token_t *parser_next(parser_t *parser)
{
    if (parser->token_index + 1 < parser->lexer->count)
    {
        ...
        parser->token_index++;
        while (parser->current_token->type < 10 && parser->token_index + 1 < parser->lexer->count)
        {
            parser->current_token = parser->lexer->tokens[parser->token_index + 1];
            parser->token_index++;
        }
...
void parser_raise(parser_t *parser,char * message,...){
    ...
    fprintf(stderr,"Error at line %d, column %d: ",parser->current_token->line,parser->current_token->col);
    }
...
token_t *parser_expect(parser_t *parser, bool required, ...)
{
    ...
    {
        parser_raise(parser,"Syntax error. Unexpected token: %s", token->value);
        exit(2);
    }
...
ast_t *parse_variable_definition(parser_t *parser)
{
    ...
    token_t *token = parser->current_token;
    bool matched_variable_identifier = string_match_option(token->value, variable_identifiers);
    if (!matched_variable_identifier)
    {
        char *user_defined_classes_string = array_to_option_string(parser->class_list);
        printf("User defined classes: %s\n", user_defined_classes_string);
        matched_variable_identifier = string_match_option(token->value, user_defined_classes_string);
...
array_t *parse_class_extends(parser_t *parser, ast_class_definition_t *definition)
{
    ...
    while (parser->current_token->type != TOKEN_PAREN_CLOSE)
    {
        if(!array_push_string(extends,parser->current_token->value)){
            parser_raise(parser,"Duplicate extend %s < %s\n",definition->name, parser->current_token->value);
        }
        parser_next(parser);
    }
...
ast_t *parse(lexer_t *lexer)
{
    ...
    {
        //while (parser->current_token && parser->current_token->type < 10)
          //  parser_next(parser);
